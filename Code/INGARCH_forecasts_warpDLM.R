...
INGfc <- foreach(j=1:numSeries) %:%
  foreach(i=itSeq, .combine=comb, .packages = c("KFAS", "truncdist", "tscount", "mvtnorm", "TruncatedNormal", "MASS"),
...
proc.time() - ptm
saveRDS(INGfc, file='./Outputs/ModelResults/simulations/INGARCH_forecasts_warpDLM_direct.rds')

INGfc <- readRDS(file="./Outputs/ModelResults/simulations/INGARCH_forecasts_warpDLM_direct.rds")
...
tempFunc <- function(par, low, up){
  locLevLike_log <- function(P0, V, W, low, up, logParams=FALSE){
    if(logParams){
      V=exp(V)
      W=exp(W)
    }
    ...
    return(-log(mvtnorm::pmvnorm(as.numeric(low), as.numeric(up), sigma=Sigma_z)[1]))
  }
  locLevLike_log(3, par[1], par[2], low, up)
}

#Redo those that had errors
cl <- makeCluster(numCores)
errors = errorSearch(INGfc)
done = (nrow(errors)==0)
while(!done){
  for(m in 1:nrow(errors)){
    ...
    if(cur[2]==1){
      ...
      sqrt_gibbs <- DLM_STAR_mcmc_KFAS(y[1:i], init_mod = init_mod, 
                                       transformation = "sqrt", y_max=ymax,
                                       update_mod = updatemod_unif_loclev_KFAS,
                                       nburn=1000, nsave=3000, nskip=0)
      a_y = a_j(y[1:i], ymax)
      a_yp1 = a_j(y[1:i] + 1, ymax)
      low = sqrt_gibbs$g_func(a_y)
      up= sqrt_gibbs$g_func(a_yp1)
      newfc <- tryCatch(expr={
        optimResults <- optimParallel(par=c(mean(sqrt_gibbs$V_post), mean(sqrt_gibbs$W_post)), fn=tempFunc, 
                                      low=low, up=up, lower=c(0, 0), upper=rep(Inf, 2), 
                                      control=list(factr=100), parallel=list(cl=cl))
        dirSampler_locLev(y[1:i], V=optimResults$par[1], W=optimResults$par[2], g=sqrt_gibbs$g_func,
                          g_inv=sqrt_gibbs$g_inv_func, y_max=ymax)
      },
      error=function(cond){
        #message(cond)
        print(paste("Error with sqrt in series", cur[1], "and time point", i))
        return(array(-999,10000))
      })
    }
    else if(cur[2]==2){
      ...
      a_y = a_j(y[1:i], ymax)
      a_yp1 = a_j(y[1:i] + 1, ymax)
      low = id_gibbs$g_func(a_y)
      up=id_gibbs$g_func(a_yp1)

      newfc <- tryCatch(expr={
        optimResults <- optimParallel(par=c(mean(id_gibbs$V_post),mean(id_gibbs$W_post)), fn=tempFunc, 
                                      low=low, up=up, lower=c(0, 0), upper=rep(Inf, 2), 
                                      control=list(factr=100), parallel=list(cl=cl))
        dirSampler_locLev(y[1:i], V=optimResults$par[1], W=optimResults$par[2], g=id_gibbs$g_func,
                          g_inv=id_gibbs$g_inv_func, y_max=ymax)
      ...
      error=function(cond){
        #message(cond)
        print(paste("Error with id in series", cur[1], "and time point", i))
        return(array(-999,10000))
      })
    }
    else if(cur[2]==3){
      ...
      a_y = a_j(y[1:i], ymax)
      a_yp1 = a_j(y[1:i] + 1, ymax)
      low = np_gibbs$g_func(a_y)
      up=np_gibbs$g_func(a_yp1)
      newfc <- tryCatch(expr={
        optimResults <- optimParallel(par=c(mean(np_gibbs$V_post),mean(np_gibbs$W_post)), fn=tempFunc, 
                              low=low, up=up, lower=c(0, 0), upper=rep(Inf, 2), 
                              control=list(factr=100), parallel=list(cl=cl))
        dirSampler_locLev(y[1:i], V=optimResults$par[1], W=optimResults$par[2], g=np_gibbs$g_func,
                          g_inv=np_gibbs$g_inv_func, y_max=ymax)
      ...
      error=function(cond){
        #message(cond)
        print(paste("Error with np in series", cur[1], "and time point", i))
        return(array(-999,10000))
      })
    }
    else{
...
saveRDS(INGfc, file='./Outputs/ModelResults/simulations/INGARCH_forecasts_warpDLM.rds')
